...
export default function DevTools({ isDark = true }) {
  ...
  const [rcmInputTab, setRcmInputTab]   = useState("text");
  const [rcmFormData, setRcmFormData]   = useState({});
  const [rcmScanStatus, setRcmScanStatus] = useState(null);
  ...
  // ── OCR: load Tesseract.js dynamically ───────────────────────────────────
  ...
  // ── Sidebar tool list (shared between desktop & mobile drawer) ───────────
        ...
        <div key={group} style={{ marginBottom:16 }}>
          ...
          {TOOLS.filter(t => t.group === group).map(t => (
            <button key={t.id} className={"dt-tool-btn" + (activeTool===t.id?" active":"")}
              onClick={() => { setActiveTool(t.id); if (isMobile) setSidebarOpen(false); }}
  ...
  // ── Render ───────────────────────────────────────────────────────────────
  return (
    <div style={{ display:"flex", height:"calc(100vh - 65px)", background:T.bg, fontFamily:"'DM Sans',sans-serif", overflow:"hidden", position:"relative" }}>
      <style>{`
        ...
        .dt-select { background:${T.inputBg}; border:1.5px solid ${T.border}; color:${T.text}; padding:8px 12px; border-radius:8px; font-family:'DM Sans',sans-serif; font-size:13px; outline:none; cursor:pointer; max-width:100%; }
        ...
        .dt-run-btn { background:linear-gradient(135deg,${tool.color},${isDark?"#6366f1":"#4f46e5"}); border:none; color:white; padding:13px 20px; border-radius:10px; cursor:pointer; font-family:'Bricolage Grotesque',sans-serif; font-weight:800; font-size:15px; transition:all 0.2s; width:100%; }
        ...
        .dt-copy-btn { padding:6px 10px; border-radius:6px; border:1.5px solid ${T.border}; background:transparent; color:${T.textMuted}; font-size:11px; cursor:pointer; font-family:'DM Sans',sans-serif; font-weight:600; transition:all 0.15s; white-space:nowrap; }
        .dt-copy-btn:hover { border-color:${tool.color}; color:${tool.color}; }
        .rcm-tab { padding:6px 12px; border-radius:8px; border:1.5px solid ${T.border}; background:transparent; color:${T.textSub}; font-size:12px; font-weight:600; cursor:pointer; font-family:'DM Sans',sans-serif; transition:all 0.15s; }
        ...
        .rcm-scan-zone { border:2px dashed ${T.border}; border-radius:12px; padding:32px 20px; display:flex; flex-direction:column; align-items:center; gap:10px; cursor:pointer; transition:all 0.2s; text-align:center; }
        ...
        .dt-hamburger { display:flex; align-items:center; justify-content:center; width:36px; height:36px; border-radius:8px; border:1.5px solid ${T.border}; background:transparent; cursor:pointer; color:${T.textSub}; font-size:18px; flex-shrink:0; }
        .dt-drawer-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.55); z-index:200; }
        .dt-drawer { position:fixed; top:0; left:0; height:100%; width:240px; background:${T.surface}; z-index:201; overflow-y:auto; box-shadow:4px 0 32px rgba(0,0,0,0.5); }
      ...
      {isMobile && sidebarOpen && (
        <>
          <div className="dt-drawer-overlay" onClick={() => setSidebarOpen(false)} />
          <div className="dt-drawer">
            <div style={{ padding:"12px 14px", borderBottom:"1px solid " + T.border, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:14, fontWeight:800, color:T.text }}>Tools</div>
              <button className="dt-hamburger" onClick={() => setSidebarOpen(false)}>✕</button>
            </div>
            <SidebarContent />
          </div>
        </>
      ...
      {!isMobile && (
        <div style={{ width:200, flexShrink:0, borderRight:"1px solid " + T.border, background:T.surface, overflowY:"auto" }}>
          <SidebarContent />
        </div>
      )}
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0 }}>
        ...
        {/* Tool header */}
        <div style={{ padding: isMobile ? "10px 12px" : "14px 20px", borderBottom:"1px solid " + T.border, background:T.surface, display:"flex", alignItems:"center", gap:10, flexShrink:0, flexWrap:"wrap" }}>
          {/* Hamburger on mobile */}
          {isMobile && (
            <button className="dt-hamburger" onClick={() => setSidebarOpen(true)}>☰</button>
          )}
          <div style={{ width:34, height:34, borderRadius:10, background:"linear-gradient(135deg," + tool.color + ",#6366f1)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:17, flexShrink:0 }}>
            {tool.icon}
          </div>
          <div style={{ minWidth:0, flex:1 }}>
            <div style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize: isMobile ? 14 : 16, fontWeight:800, color:T.text, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{tool.label}</div>
            {!isMobile && <div style={{ fontSize:11, color:T.textMuted, textTransform:"uppercase", letterSpacing:"0.8px" }}>{GROUP_LABELS[tool.group]}</div>}
          </div>

          {/* RCM mode pills — wrap below on mobile */}
          {activeTool === "rcm" && (
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", width: isMobile ? "100%" : "auto", marginTop: isMobile ? 4 : 0 }}>
          ...
          {resultVal && (
            <div style={{ marginLeft: isMobile ? 0 : "auto", display:"flex", gap:6, flexShrink:0 }}>
              ...
              <button className="dt-copy-btn" onClick={() => navigator.clipboard.writeText(resultVal)}>
        ...
        {/* Input + Result — side by side on desktop, stacked on mobile */}
        <div style={{ flex:1, display:"flex", flexDirection: isMobile ? "column" : "row", overflow: isMobile ? "auto" : "hidden" }}>
              ...
              <div style={{ flex:1, display:"flex", flexDirection:"column", gap:6 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:6 }}>
                  ...
                  <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap" }}>
                    ...
                    <button className="dt-copy-btn" onClick={() => fileInputRef.current?.click()} style={{ fontSize:10 }}>📁 Load</button>
                  </div>
                </div>
                <textarea className="dt-textarea"
                  rows={isMobile ? 8 : (cfg.rows || 14)}
          ...
          {/* Result pane */}
          <div style={{ flex:1, overflowY:"auto", padding: isMobile ? 12 : 20, minHeight: isMobile ? 200 : "auto" }}>

            {activeTool === "rcm" && !resultVal && !isRunning && !errVal && (
              <div style={{ minHeight:160, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16, opacity:0.25, padding:"20px 0" }}>
                <div style={{ fontSize: isMobile ? 40 : 56 }}>💊</div>
                <div style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize: isMobile ? 16 : 20, fontWeight:800, color:T.text }}>RCM Autonomous</div>
            ...
            {activeTool !== "rcm" && !resultVal && !isRunning && !errVal && (
              <div style={{ minHeight:160, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16, opacity:0.25, padding:"20px 0" }}>
                <div style={{ fontSize: isMobile ? 40 : 56 }}>{tool.icon}</div>
