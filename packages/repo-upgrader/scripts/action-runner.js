...
const report = await migrate(root, { target, executor: 'local', rollbackOnFailure: false, maxRepairAttempts: 1 });
...
if (report.status === 'failed') {
  console.error(`Migration transformation failed: ${report.status}`);
  process.exitCode = 2;
} else if (report.status !== 'succeeded') {
  console.warn(`Migration produced a reviewable PR with verification status: ${report.status}`);
}
