...
export function jobFromGitHubDispatch(event, payload) {
  ...
  return { source: 'github', repository: { fullName: payload.repository?.full_name, ref: payload.client_payload?.ref || payload.repository?.default_branch || 'main', installationId: payload.installation?.id }, target, deliveryId: payload.delivery_id };
}
