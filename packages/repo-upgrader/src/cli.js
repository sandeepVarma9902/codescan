...
export async function runCli(args) {
  ...
  if (command === 'migrate') {
    const report = await migrate(root, { target, skipInstall: args.includes('--skip-install'), rollbackOnFailure: args.includes('--rollback-on-failure'), force: args.includes('--force') });
